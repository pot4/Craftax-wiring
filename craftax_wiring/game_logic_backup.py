...
from craftax.craftax_classic.wire_logic import *
...
def is_game_over(state, params):
    ...
    # jax.debug.print("{}", state.half_adder_curriculum_stage)
...
def is_in_wire(state, position):
    def _is_given_wire_block(unused, block):
        return None, state.map[position[0], position[1]] == block

    _, is_in_wire = jax.lax.scan(_is_given_wire_block, None, WIRE_BLOCKS)

    return is_in_wire.sum() > 0

def is_in_wall_or_wire(state, position):
    is_in_block = is_in_wall(state, position)
    is_in_wire_block = is_in_wire(state, position)
    return jnp.logical_or(is_in_block, is_in_wire_block)
...
def do_action(rng, state, action, static_params):
    ...
    # Wire
    is_mining_wire =  state.map[block_position[0], block_position[1]] == BlockType.WIRE.value
    new_wires_positions, new_wires_charge, new_wires_mask = remove_wire(state, block_position, is_mining_wire, static_params)
    mined_wire_block = jax.lax.select(
        is_mining_wire,
        BlockType.PATH.value,
        new_map[block_position[0], block_position[1]],
    )
    new_map = new_map.at[block_position[0], block_position[1]].set(mined_wire_block)
    new_inventory = new_inventory.replace(
        wire=state.inventory.wire + 1 * is_mining_wire
    )
    state = state.replace(
        wires_positions=new_wires_positions,
        wires_charge=new_wires_charge,
        wires_mask=new_wires_mask,
    )

    # Power
    is_mining_power =  state.map[block_position[0], block_position[1]] == BlockType.POWER.value
    mined_power_block = jax.lax.select(
        is_mining_power,
        BlockType.PATH.value,
        new_map[block_position[0], block_position[1]],
    )
    new_map = new_map.at[block_position[0], block_position[1]].set(mined_power_block)
    new_inventory = new_inventory.replace(
        power=state.inventory.power + 1 * is_mining_power
    )

    # Extender
    is_mining_extender =  state.map[block_position[0], block_position[1]] == BlockType.EXTENDER.value
    new_extenders_positions, new_extenders_direction, new_extenders_mask = remove_extender(state, block_position, is_mining_extender, static_params)
    mined_extender_block = jax.lax.select(
        is_mining_extender,
        BlockType.PATH.value,
        new_map[block_position[0], block_position[1]],
    )
    new_map = new_map.at[block_position[0], block_position[1]].set(mined_extender_block)
    new_inventory = new_inventory.replace(
        extender=state.inventory.extender + 1 * is_mining_extender
    )
    state = state.replace(
        extenders_positions=new_extenders_positions,
        extenders_direction=new_extenders_direction,
        extenders_mask=new_extenders_mask,
    )

    # Junction
    is_mining_junction =  state.map[block_position[0], block_position[1]] == BlockType.JUNCTION.value
    mined_junction_block = jax.lax.select(
        is_mining_junction,
        BlockType.PATH.value,
        new_map[block_position[0], block_position[1]],
    )
    new_map = new_map.at[block_position[0], block_position[1]].set(mined_junction_block)
    new_inventory = new_inventory.replace(
        junction=state.inventory.junction + 1 * is_mining_junction
    )

    # AND
    is_mining_AND =  state.map[block_position[0], block_position[1]] == BlockType.AND.value
    new_logic_gates_positions, new_logic_gates_type, new_logic_gates_direction, new_logic_gates_power, new_logic_gates_mask = remove_logic_gate(state, block_position, is_mining_AND, static_params)
    mined_AND_block = jax.lax.select(
        is_mining_AND,
        BlockType.PATH.value,
        new_map[block_position[0], block_position[1]],
    )
    new_map = new_map.at[block_position[0], block_position[1]].set(mined_AND_block)
    new_inventory = new_inventory.replace(
        AND=state.inventory.AND + 1 * is_mining_AND
    )
    state = state.replace(
        logic_gates_positions=new_logic_gates_positions, 
        logic_gates_type=new_logic_gates_type, 
        logic_gates_direction=new_logic_gates_direction, 
        logic_gates_power=new_logic_gates_power, 
        logic_gates_mask=new_logic_gates_mask,
    )

    # OR
    is_mining_OR =  state.map[block_position[0], block_position[1]] == BlockType.OR.value
    new_logic_gates_positions, new_logic_gates_type, new_logic_gates_direction, new_logic_gates_power, new_logic_gates_mask = remove_logic_gate(state, block_position, is_mining_OR, static_params)
    mined_OR_block = jax.lax.select(
        is_mining_OR,
        BlockType.PATH.value,
        new_map[block_position[0], block_position[1]],
    )
    new_map = new_map.at[block_position[0], block_position[1]].set(mined_OR_block)
    new_inventory = new_inventory.replace(
        OR=state.inventory.OR + 1 * is_mining_OR
    )
    state = state.replace(
        logic_gates_positions=new_logic_gates_positions, 
        logic_gates_type=new_logic_gates_type, 
    ...
    # XOR
    is_mining_XOR =  state.map[block_position[0], block_position[1]] == BlockType.XOR.value
    new_logic_gates_positions, new_logic_gates_type, new_logic_gates_direction, new_logic_gates_power, new_logic_gates_mask = remove_logic_gate(state, block_position, is_mining_XOR, static_params)
    mined_XOR_block = jax.lax.select(
        is_mining_XOR,
        BlockType.PATH.value,
        new_map[block_position[0], block_position[1]],
    )
    new_map = new_map.at[block_position[0], block_position[1]].set(mined_XOR_block)
    new_inventory = new_inventory.replace(
        XOR=state.inventory.XOR + 1 * is_mining_XOR
    )
    state = state.replace(
        logic_gates_positions=new_logic_gates_positions, 
        logic_gates_type=new_logic_gates_type, 
    ...
    # NOT
    is_mining_NOT =  state.map[block_position[0], block_position[1]] == BlockType.NOT.value
    new_logic_gates_positions, new_logic_gates_type, new_logic_gates_direction, new_logic_gates_power, new_logic_gates_mask = remove_logic_gate(state, block_position, is_mining_NOT, static_params)
    mined_NOT_block = jax.lax.select(
        is_mining_NOT,
        BlockType.PATH.value,
        new_map[block_position[0], block_position[1]],
    )
    new_map = new_map.at[block_position[0], block_position[1]].set(mined_NOT_block)
    new_inventory = new_inventory.replace(
        NOT=state.inventory.NOT + 1 * is_mining_NOT
    )
    state = state.replace(
        logic_gates_positions=new_logic_gates_positions, 
        logic_gates_type=new_logic_gates_type, 
    ...
    # Pressure plate
    is_mining_pressure_plate =  state.map[block_position[0], block_position[1]] == BlockType.PRESSURE_PLATE.value
    mined_pressure_plate_block = jax.lax.select(
        is_mining_pressure_plate,
        BlockType.PATH.value,
        new_map[block_position[0], block_position[1]],
    )
    new_map = new_map.at[block_position[0], block_position[1]].set(mined_pressure_plate_block)
    new_inventory = new_inventory.replace(
        pressure_plate=state.inventory.pressure_plate + 1 * is_mining_pressure_plate
    )

    # Input
    is_mining_input =  jnp.logical_or(
        state.map[block_position[0], block_position[1]] == BlockType.INPUT_OFF.value,
        state.map[block_position[0], block_position[1]] == BlockType.INPUT_ON.value
    )
    new_inputs_positions, new_inputs_mask = remove_input(state, block_position, is_mining_input, static_params)
    mined_input_block = jax.lax.select(
        is_mining_input,
        BlockType.PATH.value,
        new_map[block_position[0], block_position[1]],
    )
    new_map = new_map.at[block_position[0], block_position[1]].set(mined_input_block)
    new_inventory = new_inventory.replace(
        input=state.inventory.input + 1 * is_mining_input
    )

    # Output
    is_mining_output =  state.map[block_position[0], block_position[1]] == BlockType.OUTPUT.value
    new_outputs_positions, new_outputs_mask = remove_output(state, block_position, is_mining_output, static_params)
    new_wires_positions, new_wires_charge, new_wires_mask = remove_wire(state, block_position, is_mining_output, static_params)
    mined_output_block = jax.lax.select(
        is_mining_output,
        BlockType.PATH.value,
        new_map[block_position[0], block_position[1]],
    )
    new_map = new_map.at[block_position[0], block_position[1]].set(mined_output_block)
    new_inventory = new_inventory.replace(
        output=state.inventory.output + 1 * is_mining_output
    )
    state = state.replace(
        wires_positions=new_wires_positions,
        wires_charge=new_wires_charge,
    ...
    state = state.replace(
        ...
        achievements=new_achievements,
        inputs_positions=new_inputs_positions,
        inputs_mask=new_inputs_mask,
        outputs_positions=new_outputs_positions,
        outputs_mask=new_outputs_mask,
    )
...
def do_crafting(state, action):
    ...
    # Wire
    can_craft_wire = new_inventory.stone >= 1
    is_crafting_wire = jnp.logical_and(
        action == Action.MAKE_WIRE.value,
        jnp.logical_and(
            can_craft_wire, is_at_crafting_table
        ),
    )
    new_inventory = new_inventory.replace(
        stone=new_inventory.stone - 1 * is_crafting_wire,
        wire=new_inventory.wire + 1 * is_crafting_wire,
    )
    new_achievements = new_achievements.at[Achievement.MAKE_WIRE.value].set(
        jnp.logical_or(
            new_achievements[Achievement.MAKE_WIRE.value], is_crafting_wire
        )
    )
    # Power
    can_craft_power = jnp.logical_and(new_inventory.stone >= 1, new_inventory.coal >= 1)
    is_crafting_power = jnp.logical_and(
        action == Action.MAKE_POWER.value,
        jnp.logical_and(
            can_craft_power, is_at_crafting_table
        ),
    )
    new_inventory = new_inventory.replace(
        stone=new_inventory.stone - 1 * is_crafting_power,
        coal=new_inventory.coal - 1 * is_crafting_power,
        power=new_inventory.power + 1 * is_crafting_power,
    )

    # Extender
    can_craft_extender = new_inventory.stone >= 1
    is_crafting_extender = jnp.logical_and(
        action == Action.MAKE_EXTENDER.value,
        jnp.logical_and(
            can_craft_extender, is_at_crafting_table
        ),
    )
    new_inventory = new_inventory.replace(
        stone=new_inventory.stone - 1 * is_crafting_extender,
        extender=new_inventory.extender + 1 * is_crafting_extender,
    )

    # Junction
    can_craft_junction = new_inventory.wire >= 1
    is_crafting_junction = jnp.logical_and(
        action == Action.MAKE_JUNCTION.value,
        jnp.logical_and(
            can_craft_junction, is_at_crafting_table
        ),
    )
    new_inventory = new_inventory.replace(
        wire=new_inventory.wire - 1 * is_crafting_junction,
        junction=new_inventory.junction + 1 * is_crafting_junction,
    )

    # AND
    can_craft_AND = new_inventory.wire >= 1
    is_crafting_AND = jnp.logical_and(
        action == Action.MAKE_AND.value,
        jnp.logical_and(
            can_craft_AND, is_at_crafting_table
        ),
    )
    new_inventory = new_inventory.replace(
        wire=new_inventory.wire - 1 * is_crafting_AND,
        AND=new_inventory.AND + 1 * is_crafting_AND,
    )

    # OR
    can_craft_OR = new_inventory.wire >= 1
    is_crafting_OR = jnp.logical_and(
        action == Action.MAKE_OR.value,
        jnp.logical_and(
            can_craft_OR, is_at_crafting_table
        ),
    )
    new_inventory = new_inventory.replace(
        wire=new_inventory.wire - 1 * is_crafting_OR,
        OR=new_inventory.OR + 1 * is_crafting_OR,
    )

    # XOR
    can_craft_XOR = new_inventory.wire >= 1
    is_crafting_XOR = jnp.logical_and(
        action == Action.MAKE_XOR.value,
        jnp.logical_and(
            can_craft_XOR, is_at_crafting_table
        ),
    )
    new_inventory = new_inventory.replace(
        wire=new_inventory.wire - 1 * is_crafting_XOR,
        XOR=new_inventory.XOR + 1 * is_crafting_XOR,
    )

    # NOT
    can_craft_NOT = new_inventory.wire >= 1
    is_crafting_NOT = jnp.logical_and(
        action == Action.MAKE_NOT.value,
        jnp.logical_and(
            can_craft_NOT, is_at_crafting_table
        ),
    )
    new_inventory = new_inventory.replace(
        wire=new_inventory.wire - 1 * is_crafting_NOT,
        NOT=new_inventory.NOT + 1 * is_crafting_NOT,
    )

    # Pressure plate
    can_craft_pressure_plate = new_inventory.wood >= 1
    is_crafting_pressure_plate = jnp.logical_and(
        action == Action.MAKE_PRESSURE_PLATE.value,
        jnp.logical_and(
            can_craft_pressure_plate, is_at_crafting_table
        ),
    )
    new_inventory = new_inventory.replace(
        wood=new_inventory.wood - 1 * is_crafting_pressure_plate,
        pressure_plate=new_inventory.pressure_plate + 1 * is_crafting_pressure_plate,
    )

    # Input
    can_craft_input = new_inventory.wire >= 1
    is_crafting_input = jnp.logical_and(
        action == Action.MAKE_INPUT.value,
        jnp.logical_and(
            can_craft_input, is_at_crafting_table
        ),
    )
    new_inventory = new_inventory.replace(
        wire=new_inventory.wire - 1 * is_crafting_input,
        input=new_inventory.input + 1 * is_crafting_input,
    )
    new_achievements = new_achievements.at[Achievement.MAKE_INPUT.value].set(
        jnp.logical_or(
            new_achievements[Achievement.MAKE_INPUT.value], is_crafting_input
        )
    )
    # Output
    can_craft_output = new_inventory.wire >= 1
    is_crafting_output = jnp.logical_and(
        action == Action.MAKE_OUTPUT.value,
        jnp.logical_and(
            can_craft_output, is_at_crafting_table
        ),
    )
    new_inventory = new_inventory.replace(
        wire=new_inventory.wire - 1 * is_crafting_output,
        output=new_inventory.output + 1 * is_crafting_output,
    )
    new_achievements = new_achievements.at[Achievement.MAKE_OUTPUT.value].set(
        jnp.logical_or(
            new_achievements[Achievement.MAKE_OUTPUT.value], is_crafting_output
        )
    )
    state = state.replace(
        inventory=new_inventory,
...
def place_block(state, action, static_params):
    ...
    is_placing_crafting_table = jnp.logical_and(
        ...
        jnp.logical_and(
            jnp.logical_not(is_in_wall_or_wire(state, placing_block_position)), has_wood
        ),
    )
    ...
    #     furnace_key_down,
    #     jnp.logical_and(
    #         jnp.logical_not(is_in_wall_or_wire(state, placing_block_position)), has_stone
    #     ),
    # )
    ...
    #     jnp.logical_not(is_in_wall_or_wire(state, placing_block_position)),
    # )
    ...
    # Wire
    wire_key_down = action == Action.PLACE_WIRE.value
    has_wire = new_inventory.wire > 0
    is_placing_on_valid_block = jnp.logical_or(
        state.map[placing_block_position[0], placing_block_position[1]]
        == BlockType.WATER.value,
        jnp.logical_not(is_in_wall_or_wire(state, placing_block_position)),
    )
    is_placing_wire = jnp.logical_and(
        wire_key_down,
        jnp.logical_and(is_placing_on_valid_block, has_wire),
    )
    placed_wire_block = jax.lax.select(
        is_placing_wire,
    ...
    new_inventory = new_inventory.replace(
        wire=new_inventory.wire - 1 * is_placing_wire
    )
    new_achievements = new_achievements.at[Achievement.PLACE_WIRE.value].set(
        jnp.logical_or(
            new_achievements[Achievement.PLACE_WIRE.value], is_placing_wire
        )
    )
    ...
    (
        ...
        state, placing_block_position, is_placing_wire, static_params
    )
    ...
    # Power
    # power_key_down = action == Action.PLACE_POWER.value
    # has_power = new_inventory.power > 0
    # is_placing_on_valid_block = jnp.logical_or(
    #     state.map[placing_block_position[0], placing_block_position[1]]
    #     == BlockType.WATER.value,
    #     jnp.logical_not(is_in_wall_or_wire(state, placing_block_position)),
    # )
    # is_placing_power = jnp.logical_and(
    #     power_key_down,
    #     jnp.logical_and(is_placing_on_valid_block, has_power),
    # )
    # placed_power_block = jax.lax.select(
    #     is_placing_power,
    ...
    # )
    # new_inventory = new_inventory.replace(
    #     power=new_inventory.power - 1 * is_placing_power
    # )

    # Extender
    # extender_key_down = action == Action.PLACE_EXTENDER.value
    # has_extender = new_inventory.extender > 0
    # is_placing_on_valid_block = jnp.logical_or(
    #     state.map[placing_block_position[0], placing_block_position[1]]
    #     == BlockType.WATER.value,
    #     jnp.logical_not(is_in_wall_or_wire(state, placing_block_position)),
    # )
    # is_placing_extender = jnp.logical_and(
    #     extender_key_down,
    #     jnp.logical_and(is_placing_on_valid_block, has_extender),
    # )
    ...
    # )
    # new_inventory = new_inventory.replace(
    #     extender=new_inventory.extender - 1 * is_placing_extender
    # )

    # (
    #     new_extenders_positions,
    #     new_extenders_direction,
    #     new_extenders_mask,
    # ) = add_new_extender(
    #     state, placing_block_position, is_placing_extender, static_params
    # )

    # Junction
    junction_key_down = action == Action.PLACE_JUNCTION.value
    has_junction = new_inventory.junction > 0
    is_placing_on_valid_block = jnp.logical_or(
        state.map[placing_block_position[0], placing_block_position[1]]
        == BlockType.WATER.value,
        jnp.logical_not(is_in_wall_or_wire(state, placing_block_position)),
    )
    is_placing_junction = jnp.logical_and(
        junction_key_down,
        jnp.logical_and(is_placing_on_valid_block, has_junction),
    )
    placed_junction_block = jax.lax.select(
        is_placing_junction,
        BlockType.JUNCTION.value,
        new_map[placing_block_position[0], placing_block_position[1]],
    )
    new_map = new_map.at[placing_block_position[0], placing_block_position[1]].set(
        placed_junction_block
    )
    new_inventory = new_inventory.replace(
        junction=new_inventory.junction - 1 * is_placing_junction
    )

    # AND
    AND_key_down = action == Action.PLACE_AND.value
    has_AND = new_inventory.AND > 0
    is_placing_on_valid_block = jnp.logical_or(
        state.map[placing_block_position[0], placing_block_position[1]]
        == BlockType.WATER.value,
        jnp.logical_not(is_in_wall_or_wire(state, placing_block_position)),
    )
    is_placing_AND = jnp.logical_and(
        AND_key_down,
        jnp.logical_and(is_placing_on_valid_block, has_AND),
    )
    placed_AND_block = jax.lax.select(
        is_placing_AND,
        BlockType.AND.value,
        new_map[placing_block_position[0], placing_block_position[1]],
    )
    ...
    new_inventory = new_inventory.replace(
        AND=new_inventory.AND - 1 * is_placing_AND
    )

    (
        new_logic_gates_positions,
        new_logic_gates_type,
        new_logic_gates_direction,
        new_logic_gates_power,
        new_logic_gates_mask,
    ) = add_new_logic_gate(
        state, placing_block_position, is_placing_AND, static_params, 1
    )
    state = state.replace(
        logic_gates_positions=new_logic_gates_positions,
        logic_gates_type=new_logic_gates_type,
    ...
    # OR
    # OR_key_down = action == Action.PLACE_OR.value
    # has_OR = new_inventory.OR > 0
    # is_placing_on_valid_block = jnp.logical_or(
    #     state.map[placing_block_position[0], placing_block_position[1]]
    #     == BlockType.WATER.value,
    #     jnp.logical_not(is_in_wall_or_wire(state, placing_block_position)),
    # )
    # is_placing_OR = jnp.logical_and(
    #     OR_key_down,
    #     jnp.logical_and(is_placing_on_valid_block, has_OR),
    # )
    # placed_OR_block = jax.lax.select(
    #     is_placing_OR,
    #     BlockType.OR.value,
    #     new_map[placing_block_position[0], placing_block_position[1]],
    # )
    ...
    # )
    # new_inventory = new_inventory.replace(
    #     OR=new_inventory.OR - 1 * is_placing_OR
    # )
    ...
    #     new_logic_gates_mask,
    # ) = add_new_logic_gate(
    #     state, placing_block_position, is_placing_OR, static_params, 2
    # )
    # state = state.replace(
    #     logic_gates_positions=new_logic_gates_positions,
    #     logic_gates_type=new_logic_gates_type,
    ...
    # XOR
    XOR_key_down = action == Action.PLACE_XOR.value
    has_XOR = new_inventory.XOR > 0
    is_placing_on_valid_block = jnp.logical_or(
        state.map[placing_block_position[0], placing_block_position[1]]
        == BlockType.WATER.value,
        jnp.logical_not(is_in_wall_or_wire(state, placing_block_position)),
    )
    is_placing_XOR = jnp.logical_and(
        XOR_key_down,
        jnp.logical_and(is_placing_on_valid_block, has_XOR),
    )
    placed_XOR_block = jax.lax.select(
        is_placing_XOR,
        BlockType.XOR.value,
        new_map[placing_block_position[0], placing_block_position[1]],
    )
    ...
    new_inventory = new_inventory.replace(
        XOR=new_inventory.XOR - 1 * is_placing_XOR
    )
    ...
    (
        ...
        state, placing_block_position, is_placing_XOR, static_params, 3
    )
    state = state.replace(
        logic_gates_positions=new_logic_gates_positions,
        logic_gates_type=new_logic_gates_type,
    ...
    # NOT
    # NOT_key_down = action == Action.PLACE_NOT.value
    # has_NOT = new_inventory.NOT > 0
    # is_placing_on_valid_block = jnp.logical_or(
    #     state.map[placing_block_position[0], placing_block_position[1]]
    #     == BlockType.WATER.value,
    #     jnp.logical_not(is_in_wall_or_wire(state, placing_block_position)),
    # )
    # is_placing_NOT = jnp.logical_and(
    #     NOT_key_down,
    #     jnp.logical_and(is_placing_on_valid_block, has_NOT),
    # )
    # placed_NOT_block = jax.lax.select(
    #     is_placing_NOT,
    #     BlockType.NOT.value,
    #     new_map[placing_block_position[0], placing_block_position[1]],
    # )
    ...
    # )
    # new_inventory = new_inventory.replace(
    #     NOT=new_inventory.NOT - 1 * is_placing_NOT
    # )
    ...
    #     new_logic_gates_mask,
    # ) = add_new_logic_gate(
    #     state, placing_block_position, is_placing_NOT, static_params, 4
    # )

    # Pressure plate
    # pressure_plate_key_down = action == Action.PLACE_PRESSURE_PLATE.value
    # has_pressure_plate = new_inventory.pressure_plate > 0
    # is_placing_on_valid_block = jnp.logical_or(
    #     state.map[placing_block_position[0], placing_block_position[1]]
    #     == BlockType.WATER.value,
    #     jnp.logical_not(is_in_wall_or_wire(state, placing_block_position)),
    # )
    # is_placing_pressure_plate = jnp.logical_and(
    #     pressure_plate_key_down,
    #     jnp.logical_and(is_placing_on_valid_block, has_pressure_plate),
    # )
    # placed_pressure_plate_block = jax.lax.select(
    #     is_placing_pressure_plate,
    #     BlockType.PRESSURE_PLATE.value,
    #     new_map[placing_block_position[0], placing_block_position[1]],
    # )
    # new_map = new_map.at[placing_block_position[0], placing_block_position[1]].set(
    #     placed_pressure_plate_block
    # )
    # new_inventory = new_inventory.replace(
    #     pressure_plate=new_inventory.pressure_plate - 1 * is_placing_pressure_plate
    # )

    # Input
    input_key_down = action == Action.PLACE_INPUT.value
    has_input = new_inventory.input > 0
    is_placing_on_valid_block = jnp.logical_or(
        state.map[placing_block_position[0], placing_block_position[1]]
        == BlockType.WATER.value,
        jnp.logical_not(is_in_wall_or_wire(state, placing_block_position)),
    )
    is_placing_input = jnp.logical_and(
        input_key_down,
        jnp.logical_and(is_placing_on_valid_block, has_input),
    )
    placed_input_block = jax.lax.select(
        is_placing_input,
    ...
    new_inventory = new_inventory.replace(
        input=new_inventory.input - 1 * is_placing_input
    )
    new_achievements = new_achievements.at[Achievement.PLACE_INPUT.value].set(
        jnp.logical_or(
            new_achievements[Achievement.PLACE_INPUT.value], is_placing_input
        )
    )
    (
        new_inputs_positions,
        new_inputs_mask,
    ) = add_new_input(
        state, placing_block_position, is_placing_input, static_params
    )

    # Output
    output_key_down = action == Action.PLACE_OUTPUT.value
    has_output = new_inventory.output > 0
    is_placing_on_valid_block = jnp.logical_or(
        state.map[placing_block_position[0], placing_block_position[1]]
        == BlockType.WATER.value,
        jnp.logical_not(is_in_wall_or_wire(state, placing_block_position)),
    )
    is_placing_output = jnp.logical_and(
        output_key_down,
        jnp.logical_and(is_placing_on_valid_block, has_output),
    )
    placed_output_block = jax.lax.select(
        is_placing_output,
    ...
    new_inventory = new_inventory.replace(
        output=new_inventory.output - 1 * is_placing_output
    )
    new_achievements = new_achievements.at[Achievement.PLACE_OUTPUT.value].set(
        jnp.logical_or(
            new_achievements[Achievement.PLACE_OUTPUT.value], is_placing_output
        )
    )
    (
        new_outputs_positions,
        new_outputs_mask,
    ) = add_new_output(
        state, placing_block_position, is_placing_output, static_params
    )
    (
        new_wires_positions,
        new_wires_charge,
...
def switch_input(state, static_params, action):
    new_map = state.map

    is_switching_input = action == Action.SWITCH_INPUT.value

    block_position = state.player_position + DIRECTIONS[state.player_direction]

    is_activating_input = jnp.logical_and(
        state.map[block_position[0], block_position[1]] == BlockType.INPUT_OFF.value,
        is_switching_input
    )
    is_deactivating_input = jnp.logical_and(
        state.map[block_position[0], block_position[1]] == BlockType.INPUT_ON.value,
        is_switching_input
    )

    did_input_switch = jnp.logical_or(is_activating_input, is_deactivating_input)
    # new_input_switched = jax.lax.select(
    #     did_input_switch,
    #     3,
    #     state.input_switched 
    # )

    new_input_block = jax.lax.select(
        is_activating_input,
    ...
    new_input_block = jax.lax.select(
        is_deactivating_input,
        BlockType.INPUT_OFF.value,
        new_input_block,
    )
    new_map = new_map.at[block_position[0], block_position[1]].set(new_input_block)
    
    new_achievements = state.achievements.at[Achievement.ACTIVATE_INPUT.value].set(
        jnp.logical_or(
            state.achievements[Achievement.ACTIVATE_INPUT.value], is_activating_input
        )
    )
    ...
    # state = jax.lax.cond(
    #     did_input_switch,
    #     lambda _: update_wiring_max(state, static_params, static_params.max_wire_charge + 1),
    #     lambda _: state,
    #     operand=None
    # )

    
    # state = binary_adder(state, static_params)

    # new_inputs_charge = update_input(state, block_position, is_switching_input)
    # state = state.replace(inputs_charge=new_inputs_charge)
    return state
...
def update_player_intrinsics(state, action):
    ...
    new_recover = jax.lax.select(new_recover < -15, 10.0, new_recover) # MODIFIED
...
def craftax_step(rng, state, action, params, static_params):
    ...
    # Switching input
    state = switch_input(state, static_params, action)
    ...
    # Wiring
    state = update_wiring_max(state, static_params)
    # state = update_all_wiring_components(state, static_params)

    # Achievements (wiring)
    # state = update_wiring_achievements(state, static_params)
    # state, reward = jax.lax.cond(
    #     state.achievements[Achievement.HALF_ADDER.value],
    #     lambda _: (state, 0.0),
    #     lambda _: half_adder_curriculum(state, static_params),
    #     operand=None
    # )
    reward = 0.0
    state, reward = half_adder_curriculum(state, static_params)
    # Intrinsics
