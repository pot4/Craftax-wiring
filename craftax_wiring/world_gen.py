...
from craftax.craftax_wiring.game_logic import calculate_light_level, get_distance_map
from craftax.craftax_wiring.envs.craftax_state import EnvState, Inventory, Mobs
from craftax.craftax_wiring.util.noise import generate_fractal_noise_2d
...
def generate_world(rng, params, static_params):
    ...
    # wires
    wires_positions = jnp.zeros(
        (static_params.max_wires, 2), dtype=jnp.int32
    )
    wires_charge = jnp.zeros(static_params.max_wires, dtype=jnp.int32)
    wires_mask = jnp.zeros(static_params.max_wires, dtype=bool)

    # extenders
    extenders_positions = jnp.zeros(
        (static_params.max_extenders, 2), dtype=jnp.int32
    )
    extenders_direction = jnp.zeros(static_params.max_extenders, dtype=jnp.int32)
    extenders_mask = jnp.zeros(static_params.max_extenders, dtype=bool)

    # logic_gates
    logic_gates_positions = jnp.zeros(
        (static_params.max_logic_gates, 2), dtype=jnp.int32
    )
    logic_gates_type = jnp.zeros(static_params.max_logic_gates, dtype=jnp.int32)
    logic_gates_direction = jnp.zeros(static_params.max_logic_gates, dtype=jnp.int32)
    logic_gates_power = jnp.zeros(static_params.max_logic_gates, dtype=bool)
    logic_gates_mask = jnp.zeros(static_params.max_logic_gates, dtype=bool)

    # inputs
    inputs_positions = jnp.zeros(
        (static_params.max_inputs, 2), dtype=jnp.int32
    )
    inputs_mask = jnp.zeros(static_params.max_inputs, dtype=bool)

    # outputs
    outputs_positions = jnp.zeros(
        (static_params.max_outputs, 2), dtype=jnp.int32
    )
    outputs_mask = jnp.zeros(static_params.max_outputs, dtype=bool)

    old_outputs=jnp.zeros(static_params.max_outputs, dtype=bool)

    truth_table_half_adder = jnp.full(4, False)

    truth_table_full_adder = jnp.full(8, False)
    
    truth_table_bin_to_gray = jnp.full(8, False)
    ...
    state = EnvState(
        ...
        wires_positions=wires_positions,
        wires_charge=wires_charge,
        wires_mask=wires_mask,
        extenders_positions=extenders_positions,
        extenders_direction=extenders_direction,
        extenders_mask=extenders_mask,
        logic_gates_positions=logic_gates_positions,
        logic_gates_type=logic_gates_type,
        logic_gates_direction=logic_gates_direction,
        logic_gates_power=logic_gates_power,
        logic_gates_mask=logic_gates_mask,
        inputs_positions=inputs_positions,
        inputs_mask=inputs_mask,
        outputs_positions=outputs_positions,
        outputs_mask=outputs_mask,
        achievements=jnp.zeros((len(Achievement),), dtype=bool),
        # input_switched=0,
        # old_outputs=old_outputs,
        truth_table_half_adder=truth_table_half_adder,
        truth_table_full_adder=truth_table_full_adder,
        truth_table_bin_to_gray=truth_table_bin_to_gray,
        half_adder_curriculum_stage=0,
