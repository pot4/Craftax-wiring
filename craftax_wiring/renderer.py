    ...
    inventory = (
        jnp.array(
            [
                ...
                state.inventory.wire,
                state.inventory.power,
                state.inventory.extender,
                state.inventory.junction,
                state.inventory.AND,
                state.inventory.OR,
                state.inventory.XOR,
                state.inventory.NOT,
                state.inventory.pressure_plate,
                state.inventory.input,
...
def render_craftax_pixels(state, block_pixel_size):
    ...
    inv_wire_texture = jax.lax.select(
        state.inventory.wire > 0,
        textures["wire_texture"],
        textures["smaller_empty_texture"],
    )
    inv_pixels = inv_pixels.at[
        block_pixel_size
        ...
        + inv_pixel_left_space : block_pixel_size * 8
        - inv_pixel_right_space,
    ].set(inv_wire_texture)
    inv_pixels = _render_number(inv_pixels, state.inventory.wire, 7, 1)
    
    inv_power_texture = jax.lax.select(
        state.inventory.power > 0,
        textures["power_texture"],
        textures["smaller_empty_texture"],
    )
    inv_pixels = inv_pixels.at[
        block_pixel_size
        ...
        + inv_pixel_left_space : block_pixel_size * 9
        - inv_pixel_right_space,
    ].set(inv_power_texture)
    inv_pixels = _render_number(inv_pixels, state.inventory.power, 8, 1)

    inv_extender_texture = jax.lax.select(
        state.inventory.extender > 0,
        textures["extender_texture"],
        textures["smaller_empty_texture"],
    )
    inv_pixels = inv_pixels.at[
        block_pixel_size * 2
        ...
        - inv_pixel_right_space,
        inv_pixel_left_space : block_pixel_size - inv_pixel_right_space,
    ].set(inv_extender_texture)
    inv_pixels = _render_number(inv_pixels, state.inventory.extender, 0, 2)

    inv_junction_texture = jax.lax.select(
        state.inventory.junction > 0,
        textures["junction_texture"],
        textures["smaller_empty_texture"],
    )
    inv_pixels = inv_pixels.at[
        block_pixel_size * 2
        ...
        + inv_pixel_left_space : block_pixel_size * 2
        - inv_pixel_right_space,
    ].set(inv_junction_texture)
    inv_pixels = _render_number(inv_pixels, state.inventory.junction, 1, 2)

    inv_AND_texture = jax.lax.select(
        state.inventory.AND > 0,
        textures["AND_texture"],
        textures["smaller_empty_texture"],
    )
    inv_pixels = inv_pixels.at[
        block_pixel_size * 2
    ...
    inv_pixels = _render_number(inv_pixels, state.inventory.AND, 2, 2)

    inv_OR_texture = jax.lax.select(
    ...
    inv_pixels = _render_number(inv_pixels, state.inventory.OR, 3, 2)

    inv_XOR_texture = jax.lax.select(
        state.inventory.XOR > 0,
        textures["XOR_texture"],
        textures["smaller_empty_texture"],
    )
    inv_pixels = inv_pixels.at[
        block_pixel_size * 2
    ...
    inv_pixels = _render_number(inv_pixels, state.inventory.XOR, 4, 2)

    inv_NOT_texture = jax.lax.select(
        state.inventory.NOT > 0,
        textures["NOT_texture"],
        textures["smaller_empty_texture"],
    )
    inv_pixels = inv_pixels.at[
        block_pixel_size * 2
    ...
    inv_pixels = _render_number(inv_pixels, state.inventory.NOT, 5, 2)

    inv_pressure_plate_texture = jax.lax.select(
        state.inventory.pressure_plate > 0,
        textures["pressure_plate_texture"],
        textures["smaller_empty_texture"],
    )
    inv_pixels = inv_pixels.at[
        block_pixel_size * 2
        ...
        + inv_pixel_left_space : block_pixel_size * 7
        - inv_pixel_right_space,
    ].set(inv_pressure_plate_texture)
    inv_pixels = _render_number(inv_pixels, state.inventory.pressure_plate, 6, 2)

    inv_input_texture = jax.lax.select(
        state.inventory.input > 0,
        textures["input_off_texture"],
        textures["smaller_empty_texture"],
    )
    inv_pixels = inv_pixels.at[
        block_pixel_size * 2
        ...
        + inv_pixel_left_space : block_pixel_size * 8
        - inv_pixel_right_space,
    ].set(inv_input_texture)
    inv_pixels = _render_number(inv_pixels, state.inventory.input, 7, 2)

    inv_output_texture = jax.lax.select(
        state.inventory.output > 0,
    ...
    inv_pixels = inv_pixels.at[
        ...
        + inv_pixel_left_space : block_pixel_size * 9
        - inv_pixel_right_space,
    ].set(inv_output_texture)
    inv_pixels = _render_number(inv_pixels, state.inventory.output, 8, 2)
